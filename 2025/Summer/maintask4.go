...
func main() {
	...
	game.CurrentStrategy = game.SelectStrategy()
...
// Boss4Strategy - Simple hardcoded strategy for bomb-only league
type Boss4Strategy struct{}

func (s *Boss4Strategy) Name() string {
	return "Boss4"
}

func (s *Boss4Strategy) EvaluateActions(agent *Agent, game *Game) []AgentAction {
	actions := []AgentAction{}

	// Calculate cluster centers dynamically based on map size
	clusters := [][]int{
		{2, 2},                            // Top-left
		{game.Width - 3, 2},               // Top-right
		{2, game.Height - 3},              // Bottom-left
		{game.Width - 3, game.Height - 3}, // Bottom-right
	}

	fmt.Fprintln(os.Stderr, fmt.Sprintf("Map size: %dx%d, Calculated clusters: %v", game.Width, game.Height, clusters))

	// Find both agents
	agent1X, agent1Y := -1, -1
	agent2X, agent2Y := -1, -1
	for _, a := range game.Agents {
		if a.Player == game.MyID {
			if a.ID == 1 {
				agent1X, agent1Y = a.X, a.Y
			} else if a.ID == 2 {
				agent2X, agent2Y = a.X, a.Y
			}
		}
	}

	// Determine which agent is closest to ANY cluster (that one hunkers down)
	hunkerAgentID := -1
	minDistanceToAnyCluster := 999

	// Check Agent 1's closest cluster
	if agent1X != -1 {
		for _, cluster := range clusters {
			distance := abs(agent1X-cluster[0]) + abs(agent1Y-cluster[1])
			if distance < minDistanceToAnyCluster {
				minDistanceToAnyCluster = distance
				hunkerAgentID = 1
			}
		}
	}

	// Check Agent 2's closest cluster
	if agent2X != -1 {
		for _, cluster := range clusters {
			distance := abs(agent2X-cluster[0]) + abs(agent2Y-cluster[1])
			if distance < minDistanceToAnyCluster {
				minDistanceToAnyCluster = distance
				hunkerAgentID = 2
			}
	...
	fmt.Fprintln(os.Stderr, fmt.Sprintf("Agent %d is closest to clusters (distance %d), will hunker down", hunkerAgentID, minDistanceToAnyCluster))

	if agent.ID == hunkerAgentID {
		// This agent hunkers down
		actions = append(actions, AgentAction{
			Type:     ActionHunker,
			Priority: PriorityDefault,
			Reason:   fmt.Sprintf("Boss4: Agent %d hunkering down (closest to clusters)", agent.ID),
		})
	} else {
		// This agent bombs all clusters except the one the hunker agent is in
		if agent.SplashBombs > 0 {
			// Find which cluster the hunker agent is in
			hunkerClusterX, hunkerClusterY := -1, -1
			hunkerX, hunkerY := agent2X, agent2Y
			if hunkerAgentID == 1 {
				hunkerX, hunkerY = agent1X, agent1Y
			}

			if hunkerX != -1 {
				for _, cluster := range clusters {
					distance := abs(hunkerX-cluster[0]) + abs(hunkerY-cluster[1])
					if distance <= 6 { // Close enough to be "in" this cluster
						hunkerClusterX, hunkerClusterY = cluster[0], cluster[1]
						fmt.Fprintln(os.Stderr, fmt.Sprintf("Hunker agent at (%d,%d) is in cluster (%d,%d)", hunkerX, hunkerY, hunkerClusterX, hunkerClusterY))
						break
					}
				}
			}

			// Find closest bombable cluster (excluding hunker agent's cluster)
			bestClusterX, bestClusterY := -1, -1
			minDistance := 999

			fmt.Fprintln(os.Stderr, fmt.Sprintf("Agent %d bombing, avoiding cluster (%d,%d)", agent.ID, hunkerClusterX, hunkerClusterY))

			for _, cluster := range clusters {
				clusterX, clusterY := cluster[0], cluster[1]

				// Skip hunker agent's cluster
				if clusterX == hunkerClusterX && clusterY == hunkerClusterY {
					fmt.Fprintln(os.Stderr, fmt.Sprintf("Skipping hunker cluster (%d,%d)", clusterX, clusterY))
					continue
				}

				// Check if this cluster has any living enemies
				hasEnemies := false
				for dy := -2; dy <= 2; dy++ {
					for dx := -2; dx <= 2; dx++ {
						checkX, checkY := clusterX+dx, clusterY+dy
						if game.IsValidPosition(checkX, checkY) {
				...
				if !hasEnemies {
					fmt.Fprintln(os.Stderr, fmt.Sprintf("Cluster (%d,%d) is empty, skipping", clusterX, clusterY))
					continue
				}

				distance := abs(agent.X-clusterX) + abs(agent.Y-clusterY)
				fmt.Fprintln(os.Stderr, fmt.Sprintf("Cluster (%d,%d) has enemies, distance: %d", clusterX, clusterY, distance))

				if distance <= 4 && distance < minDistance {
					bestClusterX, bestClusterY = clusterX, clusterY
					minDistance = distance
					fmt.Fprintln(os.Stderr, fmt.Sprintf("New best bombing target: (%d,%d)", bestClusterX, bestClusterY))
				}
			}

			// If we can bomb a cluster, do it
			if bestClusterX != -1 {
				fmt.Fprintln(os.Stderr, fmt.Sprintf("Agent %d BOMBING cluster (%d,%d)", agent.ID, bestClusterX, bestClusterY))
				actions = append(actions, AgentAction{
					Type:     ActionThrow,
					TargetX:  bestClusterX,
					TargetY:  bestClusterY,
					Priority: PriorityCombat,
					Reason:   fmt.Sprintf("Boss4: Agent %d bombing cluster (%d,%d)", agent.ID, bestClusterX, bestClusterY),
				})
			} else {
				// Move toward closest valid cluster for bombing
				closestClusterX, closestClusterY := 2, 2
				minDistance := 999
				...
				for _, cluster := range clusters {
					...
					if !hasEnemies {
						fmt.Fprintln(os.Stderr, fmt.Sprintf("Movement: Cluster (%d,%d) is empty, skipping", clusterX, clusterY))
						continue
					}

					distance := abs(agent.X-clusterX) + abs(agent.Y-clusterY)
					if distance < minDistance {
						closestClusterX, closestClusterY = clusterX, clusterY
						minDistance = distance
						fmt.Fprintln(os.Stderr, fmt.Sprintf("Movement: Found cluster (%d,%d) with enemies at distance %d", clusterX, clusterY, distance))
					}
				}

				// Check if we found any valid clusters with enemies
				if minDistance == 999 {
					fmt.Fprintln(os.Stderr, fmt.Sprintf("Agent %d: All enemy clusters cleared! Mission complete.", agent.ID))
					// No more enemy clusters, mission complete - just hunker down
					actions = append(actions, AgentAction{
						Type:     ActionHunker,
						Priority: PriorityDefault,
						Reason:   fmt.Sprintf("Boss4: Agent %d mission complete - all clusters cleared", agent.ID),
					})
				} else {
					// Find optimal bombing position for closest cluster (within 4 tiles of cluster)
					bestBombPosX, bestBombPosY := closestClusterX, closestClusterY
					minDistanceToAgent := 999

					// Check positions around the cluster that are within bomb range (4 tiles)
					for dy := -4; dy <= 4; dy++ {
						for dx := -4; dx <= 4; dx++ {
							bombPosX, bombPosY := closestClusterX+dx, closestClusterY+dy

							// Must be within bomb range of cluster
							if abs(dx)+abs(dy) > 4 {
								continue
							}

							// Must be valid position
							if !game.IsValidPosition(bombPosX, bombPosY) || game.Grid[bombPosY][bombPosX].Type > 0 {
								continue
							}

							// Find closest bomb position to agent
							distanceToAgent := abs(agent.X-bombPosX) + abs(agent.Y-bombPosY)
							if distanceToAgent < minDistanceToAgent {
								bestBombPosX, bestBombPosY = bombPosX, bombPosY
								minDistanceToAgent = distanceToAgent
							}
						}
					}

					// Move toward optimal bombing position (up to 2 tiles)
					deltaX, deltaY := 0, 0
					if bestBombPosX > agent.X {
						deltaX = min(2, bestBombPosX-agent.X)
					} else if bestBombPosX < agent.X {
						deltaX = max(-2, bestBombPosX-agent.X)
					}
					if bestBombPosY > agent.Y {
						deltaY = min(2, bestBombPosY-agent.Y)
					} else if bestBombPosY < agent.Y {
						deltaY = max(-2, bestBombPosY-agent.Y)
					}

					// Try primary movement direction
					targetX, targetY := agent.X+deltaX, agent.Y+deltaY
					moved := false

					if game.IsValidPosition(targetX, targetY) && game.Grid[targetY][targetX].Type == 0 {
						fmt.Fprintln(os.Stderr, fmt.Sprintf("Agent %d moving toward bombing position (%d,%d) for cluster (%d,%d)", agent.ID, bestBombPosX, bestBombPosY, closestClusterX, closestClusterY))
						actions = append(actions, AgentAction{
							Type:     ActionMove,
							TargetX:  targetX,
							TargetY:  targetY,
							Priority: PriorityMovement,
							Reason:   fmt.Sprintf("Boss4: Agent %d moving toward bombing position (%d,%d)", agent.ID, bestBombPosX, bestBombPosY),
						})
						moved = true
					} else {
						// Primary direction blocked, try alternatives
						fmt.Fprintln(os.Stderr, fmt.Sprintf("Primary move to (%d,%d) blocked, trying alternatives", targetX, targetY))
						...
						for _, alt := range alternatives {
							altX, altY := agent.X+alt[0], agent.Y+alt[1]
							if game.IsValidPosition(altX, altY) && game.Grid[altY][altX].Type == 0 {
								fmt.Fprintln(os.Stderr, fmt.Sprintf("Agent %d alternative move to (%d,%d)", agent.ID, altX, altY))
								actions = append(actions, AgentAction{
									Type:     ActionMove,
									TargetX:  altX,
									TargetY:  altY,
									Priority: PriorityMovement,
									Reason:   fmt.Sprintf("Boss4: Agent %d alternative move toward bombing position (%d,%d)", agent.ID, bestBombPosX, bestBombPosY),
								})
								moved = true
								break
							}
					...
					if !moved {
						fmt.Fprintln(os.Stderr, fmt.Sprintf("Agent %d completely blocked, no movement possible", agent.ID))
					}
				}
			}
		}
...
func (s *TakeCoverAndShootBombStrategy) EvaluateActions(agent *Agent, game *Game) []AgentAction {
	// Check if this is Boss 4 (bomb-only league with many enemies)
	if agent.SplashBombs > 0 {
		// Count total enemies to see if this is Boss 4 scenario
		totalEnemies := 0
		for _, enemy := range game.Agents {
			if enemy.Player != game.MyID && enemy.Wetness < 100 {
				totalEnemies++
		...
		// Boss 4 typically has 24+ enemies at start, use hardcoded strategy
		if totalEnemies >= 20 {
			fmt.Fprintln(os.Stderr, fmt.Sprintf("Agent %d detected Boss4 scenario (%d enemies) - using hardcoded strategy", agent.ID, totalEnemies))
			boss4Strategy := &Boss4Strategy{}
			return boss4Strategy.EvaluateActions(agent, game)
		}
	}

	// Fall back to two-phase strategy for other scenarios
...
func (game *Game) SelectStrategy() Strategy {
	// Hardcode Boss 4 strategy detection for this specific league
	// Boss 4 has exactly 4 clusters of enemies and shooting is disabled
	fmt.Fprintln(os.Stderr, "HARDCODED: Using Boss4Strategy for this league")
	return &Boss4Strategy{}
}
